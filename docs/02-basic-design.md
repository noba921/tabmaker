...
| モード | 処理 | 所要時間 | 用途 |
|---|---|---|---|
| 譜面を作る | 取得 → 分離 → 拍 → 採譜 → 保存 | 数分〜十数分 | 通常の採譜 |
| 音源だけ保存 | 取得 → 形式変換 | 数十秒 | **他のアプリで音を解析する場合** |

音源だけ保存するモードは `audio/` に置くだけで、`songs/` には何も作らない。
ジョブの構造(段階・進捗・エラー)は共通なので、UIは同じ進捗表示を使い回す。
形式の既定はWAV(非圧縮)——音色そのものを扱う用途では、非可逆圧縮が
高域の歪み方を変えてしまうため。
...
├── audio/                      採譜せず取り込んだ音源(git管理外)
│   └── <曲名>.wav              他のアプリからパスで直接読める置き場
...
| POST | `/api/audio/youtube` | **採譜せず**音源だけ取り込む `{url, format}` |
| POST | `/api/audio/upload` | 同上(ファイルから) |
| GET | `/api/audio` | 取り込んだ音源の一覧 `{files, total_size, dir}` |
| GET | `/api/audio/file/<name>` | 音源のダウンロード |
| DELETE | `/api/audio/<name>` | 音源の削除 |
